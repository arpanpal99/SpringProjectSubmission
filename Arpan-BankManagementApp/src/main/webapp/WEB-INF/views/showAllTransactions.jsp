...
<body>
	...
	<div class="operations">
		<a href="../transaction/transfer"><button
				class="op button button2">TRANSFER</button></a> <a
			href="../transaction/deposit"><button class="op button button2">DEPOSIT</button></a>
		<a href="../transaction/withdraw"><button
				class="op button button2">WITHDRAW</button></a> <a
			href="../transaction/pendingTransaction"><button
				class="op button button3">Show Pending Transactions</button></a>
	</div>
	<div>
		...
		<tbody>
			<c:forEach items="${transactions}" var="transaction">
				<tr>
					...
					<td><c:if test="${transaction.txFromAccountId==null}">
							self
						</c:if> <c:if test="${transaction.txFromAccountId!=null}">
							${transaction.txFromAccountId}
						</c:if></td>
					<td><c:if test="${transaction.txToAccountId==null}">
							self
						</c:if> <c:if test="${transaction.txToAccountId!=null}">
